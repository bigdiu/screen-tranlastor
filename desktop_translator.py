...
# -*- coding: utf-8 -*-
"""
Desktop Translator v1 - 截图翻译桌面工具
=========================================
快捷键框选屏幕区域 → OCR 识别英文/中文 → 百度翻译(自动降级必应) → 浮窗显示
...
PyInstaller 打包:
    ...
    pyinstaller --noconsole --onefile --icon=myicon.ico desktop_translator.py
...
class TranslatorService:
    """使用 translators 库, 优先百度, 失败降级到 Bing"""
    ...
    def translate(cls, text: str, cache: TranslationCache | None = None) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Google 翻译失败: {str(e)[:60]}")
...
class TranslatorApp:
    ...
    def self_test(self):
        ...
        # Step 3: 翻译
        print("[3/3] Google 翻译 ...", end=" ", flush=True)
        t0 = time.time()
        try:
