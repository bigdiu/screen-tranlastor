...
生成的 `DesktopTranslator.exe` 在 `dist/` 目录，**需要与 `tesseract/` 文件夹放在同一目录**（去 [Tesseract 安装目录](https://github.com/UB-Mannheim/tesseract/wiki) 复制 `tesseract.exe` 及其语言数据 `tessdata/` 即可）。
